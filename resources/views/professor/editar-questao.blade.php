<x-app-layout>
            ...
            <div class="card-body p-4">
                <form method="POST" action="{{ route('professor.banco-questoes.update', $questao) }}" enctype="multipart/form-data">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select" id="disciplina_id" name="disciplina_id" required>
                                @foreach ($disciplinas as $disciplina)
                                    <option value="{{ $disciplina->id }}" @if($disciplina->id == old('disciplina_id', $questao->disciplina_id)) selected @endif>
                                        {{ $disciplina->nome }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select" id="serie_id" name="serie_id" required>
                                 @foreach ($series as $serie)
                                    <option value="{{ $serie->id }}" @if($serie->id == old('serie_id', $questao->serie_id)) selected @endif>
                                        {{ $serie->nome }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div id="opcoes_multipla_escolha">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3"><label for="opcao_a" class="form-label">Opção A</label><textarea class="form-control" name="opcao_a" id="opcao_a">{{ old('opcao_a', $questao->opcao_a) }}</textarea></div>
                            <div class="col-md-6 mb-3"><label for="opcao_b" class="form-label">Opção B</label><textarea class="form-control" name="opcao_b" id="opcao_b">{{ old('opcao_b', $questao->opcao_b) }}</textarea></div>
                        </div>
                        <div class="row">
                            <div class="col-md-6 mb-3"><label for="opcao_c" class="form-label">Opção C</label><textarea class="form-control" name="opcao_c" id="opcao_c">{{ old('opcao_c', $questao->opcao_c) }}</textarea></div>
                            <div class="col-md-6 mb-3"><label for="opcao_d" class="form-label">Opção D</label><textarea class="form-control" name="opcao_d" id="opcao_d">{{ old('opcao_d', $questao->opcao_d) }}</textarea></div>
                        </div>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        @if ($questao->imagem_nome)
                            <div class="mt-2">
                                ...
                                <img src="{{ asset('storage/uploads/questoes/' . $questao->imagem_nome) }}" alt="Imagem da questão" style="max-width: 200px; max-height: 200px; border-radius: 5px; border: 1px solid #ddd;">
                            </div>
                        @endif
                    </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const opcoesContainer = document.getElementById('opcoes_multipla_escolha');

        function toggleOpcoes() {
            opcoesContainer.style.display = (tipoSelect.value === 'multipla_escolha') ? 'block' : 'none';
        }
        toggleOpcoes(); // Executa na carga da página
        tipoSelect.addEventListener('change', toggleOpcoes); // Adiciona o listener
    });
    </script>
    @endpush
</x-app-layout>